...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2,stratify=y,random_state=211) #211
...
#param_grid = {
#    'bootstrap': [True],
#    'max_depth': [5,10,15,20,25,30,35,40], ##40
#    'max_features': [5,6,7,8,9,10,11,12], ##11
#    'min_samples_leaf': [5,6,7,8,9,10], ##50
#    'min_samples_split': [5,6,7,8,9,10], ##8
#    'n_estimators': [30,40,50,60,70,80] ##33
#}


###Begin For mutants having high accuracy#####
param_grid = {
    'bootstrap': [True],
    'max_depth': [17], ## we kept a balance between accuracy and overfitting (difference between training and test/validation loss)
    'max_features': [28], ##
    'min_samples_leaf': [6], ##5
    'min_samples_split': [10], ##8
    'n_estimators': [55] ##33, we got 55, so we are check the boundry values
}
###End For mutants having high accuracy#####
